                ...
                attributes: {
                  ...
                  conform: function(attributes) {
                    return isAttributeIncluded(attributes,
                      ...
                      isValidAttributeValues(caConfig.BUTTON_ENTITY.TYPE, attributes);
                  },
 ...
 */
function isValidAttributeValues(entityType, attributes) {
  ...
  } else if (entityType === caConfig.SERVICE_ENTITY.INTERACTION_TYPE_ATTR_NAME) {
  ...
  for (var i = 0; i < attributes.length; i++) {
    ...
    if (!attributes[i].value || !attributes[i].value.length || attributes[i].value === 'null') {
      return false;
    }
  }
  return true;
                    ...
                    attributes: {
                      ...
                      conform: function(attributes) {
                        return isAttributeIncluded(
                          ...
                          isValidAttributeValues(caConfig.SERVICE_ENTITY.TYPE, attributes);
                      },
                      message: 'lacks some mandatory attributes or invalid values are included'
                    }
                  }
...
function getThirdPartyPayload(serviceDescriptor, operationDescriptor) {
  var payload;
  if (serviceDescriptor.interactionType === caConfig.SERVICE_ENTITY.INTERACTION_TYPES.SYNCHRONOUS) {
    // TODO For the time being we return the operation descriptor itself
    return operationDescriptor;
  } else if (serviceDescriptor.interactionType ===
      caConfig.SERVICE_ENTITY.INTERACTION_TYPES.ASYNCHRONOUS) {
    payload = operationDescriptor;
    payload.callbackUrl = 'http://' + caConfig.CA_HOST + ':' + caConfig.CA_PORT +
...
function getResult(err, data) {
  ...
  if (data) {
    if (data.body) {
      detailsObj = JSON.parse(data.body.details);
    ...
    for (var key in detailsObj) {
      if (detailsObj.hasOwnProperty(key)) {
        result += (escapeResultData(key) + '-' + escapeResultData(detailsObj[key]) + ';');
      }
    }
    return '1,' + result;
  } else {
...
/**
 * Escapes the data to be included into the result. More precisely the following characters: [|-,#]
 * @param {string} data The string to be escaped
 * @return {string} The escaped string
 */
function escapeResultData(data) {
  return data.replace(/\[/, '\\[').
    replace(/]/g, '\\]').
    replace(/-/g, '\\-').
    replace(/,/g, '\\,').
...
function sendThirdPartyRequest(updateContextRequest, callback) {
  ...
  caLogger.info(
    ...
    'Sending ' + updateContextRequest.contextAdapter.serviceDescriptor.method + ' request to third party at: ' +
      updateContextRequest.contextAdapter.serviceDescriptor.endpoint + ' with payload: ' +
      JSON.stringify(payload)
  );
...
function notifyOperationCompleted(request, data, callback) {
  ...
  caLogger.info(
    ...
    'Notifying the Context Broker that the request has been completed with payload: ' +
    JSON.stringify(payload)
  );

  sendUpdateContext(
    request ? request.headers['fiware-service'] : data.headers['fiware-service'],
    request ? request.headers['fiware-servicepath'] : data.headers['fiware-service'],
    payload,
    callback
  );
}
...
function notifyOperationClosed(err, request, callback) {
  ...
  caLogger.info(
    request.contextAdapter.context,
    'Notifying the Context Broker that the request has been closed with payload: ' +
    JSON.stringify(payload)
  );

  ...
  if (data) {
    ...
    for (var key in detailsObj) {
      if (detailsObj.hasOwnProperty(key)) {
        result += (encodeResultData(key) + '-' + encodeResultData(detailsObj[key]) + ';');
      }
    }
    return '1,' + result;
  } else {
...
/**
 * Encodes the data to be included into the result. More precisely the following characters: [|-,#] and the forbidden
 *  characters imposed by the Context Broker
...
function encodeResultData(data) {
  return data.toString().replace(/\[/, '\\[').
    ...
    replace(/\|/g, '\u007c').
    replace(/;/, '\u003b').
    // Escaping forbidden characters imposed by the Context Broker
    replace(/</g, '\u003c').
    replace(/>/g, '\u003e').
    replace(/"/g, '\u0022').
    replace(/'/g, '\u0027').

    ...
    if (err) {
      ...
      request.payload.contextElements[0].attributes.forEach(function(attribute) {
        switch (attribute.name) {
          case caConfig.BUTTON_ENTITY.CA_OPERATION_INTERACTION_TYPE_ATTR_NAME:
...
function getThirdPartyPayload(serviceDescriptor, operationDescriptor) {
  var payload;
  if (operationDescriptor.interactionType === caConfig.BUTTON_ENTITY.OPERATION_INTERACTION_TYPE.SYNCHRONOUS) {
    // TODO For the time being we return the operation descriptor itself
    return operationDescriptor;
  } else if (operationDescriptor.interactionType ===
...
function notifyOperationCompleted(request, response, callback) {
  ...
  if (request && request.contextAdapter && request.operationDescriptor &&

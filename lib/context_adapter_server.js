...
function respond2UpdateContext(request, reply, operationDescriptor, callback) {
  if (caComm.isUpdateContext(request)) {
...
function updateContextErrorHandler(request, reply, err) {
  ...
  if (caComm.isUpdateContext(request) && (err instanceof caErrors.BadPayload)) {
    // The request is an updateContext one but the payload is invalid (the operation description could not be

...
function validateHeaders(value, options, next) {
  ...
  var context = {
    corr: caHelper.getCorrelator(value),
    trans: caHelper.createTransaction(),
    op: caHelper.getOperationType(),
    srv: value[caConfig.HEADER.FIWARE_SERVICE],
    subsrv: value[caConfig.HEADER.FIWARE_SERVICE_PATH]
  };

  attendedRequests++;

  if (!value[caConfig.HEADER.FIWARE_SERVICE]) {
    message = 'error=child "' + caConfig.HEADER.FIWARE_SERVICE + '" fails because [' + caConfig.HEADER.FIWARE_SERVICE +
      ' is required]';
    caLogger.warn(
      context,
      message
    ...
    error.output.payload.validation = {source: 'headers', keys: [caConfig.HEADER.FIWARE_SERVICE]};
    next(error);
  } else if (!value[caConfig.HEADER.FIWARE_SERVICE_PATH]) {
    message = 'child "' + caConfig.HEADER.FIWARE_SERVICE_PATH + '" fails because [' +
      caConfig.HEADER.FIWARE_SERVICE_PATH + ' is required]';
    caLogger.warn(
      context,
      message
    );
    error = boom.badRequest(message);
    error.output.payload.validation = {source: 'headers', keys: [caConfig.HEADER.FIWARE_SERVICE_PATH]};
    next(error);
  }
  next();
}
...
function getVersionHandler(request, reply) {
  var response,
      message = caHelper.getVersion();
  response = reply(message);
  if (caHelper.getCorrelator(request)) {
    response.header(caConfig.HEADER.CORRELATOR, caHelper.getCorrelator(request));
  }
}
...
function respond2Request(request, reply, callback) {
  ...
  var response,
      ngsiResponse = caComm.getNGSIResponse(null, request);
  caLogger.debug(
...
function requestErrorHandler(request, reply, err) {
  var response, ngsiResponse;

  caLogger.warn(
    request.contextAdapter.context,
    'Some error occurred when processing the request: ' +
    (err.code || err.name) + ' - ' + err.message + '. The request will be ignored.'
  );

  if (err instanceof caErrors.BadOperationStatus || err instanceof caErrors.BadPayload) {
    ngsiResponse = caComm.getNGSIResponse(err, request);
    response = reply(ngsiResponse);
    if (caHelper.getCorrelator(request)) {
      response.header(caConfig.HEADER.CORRELATOR, caHelper.getCorrelator(request));
    }
    ...
    if (request.contextAdapter.serviceDescriptor.interactionType ===
      ...
      caComm.notifyOperationCompletedSync(
        ...
        function onNotifyOperationCompleted(err, response, body) {
          if (body && body.errorCode && body.errorCode.code !== '200') {
            caLogger.warn(
              request.contextAdapter.context,
...
function getContext(request) {
  return {
    corr: caHelper.getCorrelator(request),
    trans: caHelper.createTransaction(),
    op: caHelper.getOperationType(request),
    srv: request.headers[caConfig.HEADER.FIWARE_SERVICE],
    subsrv: request.headers[caConfig.HEADER.FIWARE_SERVICE_PATH]
  };
}
...
function queryContextHandler(request, reply) {
  ...
  response = reply(caComm.getQueryContextNGSIResponse(request));
  if (caHelper.getCorrelator(request)) {
    response.header(caConfig.HEADER.CORRELATOR, caHelper.getCorrelator(request));
  }
}
...
function start(host, port, callback) {
  ...
  server.route([
    ...
    {
      method: 'PUT',
      path: caConfig.CA_ROUTE_SUFFIXES.ADMIN_SET_LOG_LEVEL,

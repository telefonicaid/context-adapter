...
function checkNotificationRequest(request, callback) {
  function attributesConform(attributes) {
    return caHelper.isAttributeIncluded(attributes,
      ...
      (caHelper.getAttributeValue(attributes, caConfig.BUTTON_ENTITY.INTERACTION_TYPE_ATTR_NAME) ===
      caConfig.BUTTON_ENTITY.INTERACTION_TYPES.SYNCHRONOUS ||
      caHelper.isAttributeIncluded(attributes, caConfig.BUTTON_ENTITY.OPERATION_STATUS_ATTR_NAME)) &&
      caHelper.isValidAttributeValues(
        request.contextAdapter.context, caConfig.BUTTON_ENTITY.TYPE, attributes);
  }
...
/**
 * Returns true if the request is an updateContext request
 * @param {object} request The received request
 * @return {boolean} true if the request is an updateContext request, false otherwise
 */
function isUpdateContext(request) {
...
function isPollingUpdateContext(request) {
  if (isUpdateContext(request) && caHelper.getAttributeValue(
      request.payload.contextElements[0].attributes, caConfig.BUTTON_ENTITY.CA_LAST_OPERATION) === 'P') {
      return true;
  }
  return false;
...
function sendThirdPartyRequest(request, callback) {
  var payload = getThirdPartyPayload(request.contextAdapter.serviceDescriptor,
    request.contextAdapter.operationDescriptor);
    ...
    contextElements[0].attributes.push(
      {
        ...
        value: encodeResponse(JSON.stringify(getResult(null, response)))
      }
    );
  }
}
  ...
  var contextElements = [
    {
      ...
      attributes: [
        ...
        },
        {
          name: caConfig.BUTTON_ENTITY.OPERATION_RESULT_ATTR_NAME,
          type: caConfig.ATTRIBUTE_TYPE.STRING,
          value: encodeResponse(JSON.stringify(getResult(null, thirdPartyUpdateRequest)))
        }
      ]
    }
  ];
    ...
    contextElements: [
      {
        ...
        attributes: [
          ...
          },
          {
            name: caConfig.BUTTON_ENTITY.OPERATION_RESULT_ATTR_NAME,
            type: caConfig.ATTRIBUTE_TYPE.STRING,
            value: encodeResponse(JSON.stringify(getResult(err)))
          }
        ]
      }
    ],
...
function getUpdateContextNGSIResponse(err, request) {
  ...
  if (err) {
    statusCode = {
      code: '400',
      reasonPhrase: encodeResponse((err.code || err.name) + ' - ' + err.message)
    };
  } else {
    statusCode = {
      code: '200',
...
module.exports = {
  isUpdateContext: isUpdateContext,

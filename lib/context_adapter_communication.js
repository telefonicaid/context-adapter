 ...
 * @param {object} request The original HTTP request
 ...
 */
function sendQueryContext(request, payload, callback) {
  var headers = {
    'Accept': 'application/json',
    'Content-Type': 'application/json',
    'Fiware-Service': request.headers[caConfig.HEADER.FIWARE_SERVICE],
    'Fiware-ServicePath': request.headers[caConfig.HEADER.FIWARE_SERVICE_PATH]
  };
  if (caHelper.getCorrelator(request)) {
    headers['Fiware-Correlator'] = caHelper.getCorrelator(request);
  } else {
    headers['Fiware-Correlator'] = caHelper.createCorrelator();
  }
  var requestOptions = {
    method: 'POST',
  ...
  caLogger.debug(
    ...
    'Fiware-Service: ' + headers[caConfig.HEADER.FIWARE_SERVICE] + ' and ' +
    'Fiware-ServicePath: ' + headers[caConfig.HEADER.FIWARE_SERVICE_PATH] + ' and ' +
    'Fiware-Correlator: ' + headers[caConfig.HEADER.CORRELATOR] +
    ' and using payload: ' +
    ' with payload: ' +
 ...
 *  the obtained response
 */
function sendUpdateContext(request, payload, callback) {
  var headers = {
    'Accept': 'application/json',
    'Content-Type': 'application/json',
    'Fiware-Service': request.headers[caConfig.HEADER.FIWARE_SERVICE],
  ...
  caLogger.debug(
    ...
    ' with headers: ' +
    'Fiware-Service: ' + requestOptions.headers[caConfig.HEADER.FIWARE_SERVICE] + ' and ' +
    'Fiware-ServicePath: ' + requestOptions.headers[caConfig.HEADER.FIWARE_SERVICE_PATH] + ' and ' +
    'Fiware-Correlator: ' + caHelper.getCorrelator(requestOptions) +
...
function checkNotificationRequest(request, callback) {
  ...
  if (validation.valid) {
    ...
    if (operationStatus === caConfig.BUTTON_ENTITY.OPERATION_STATUS.PENDING ||
      operationStatus === caConfig.BUTTON_ENTITY.OPERATION_STATUS.REQUESTED) {
      return process.nextTick(callback);
    } else {
...
function sendThirdPartyRequest(request, callback) {
  ...
  caLogger.debug(
    ...
    'Fiware-Service: ' + request.headers[caConfig.HEADER.FIWARE_SERVICE] + ' and ' +
    'Fiware-ServicePath: ' + request.headers[caConfig.HEADER.FIWARE_SERVICE_PATH] + ' and ' +
    'Fiware-Correlator: ' + caHelper.getCorrelator(request) +

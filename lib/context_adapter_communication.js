...
function checkNotificationRequest(request, callback) {
  ...
  if (validation.valid) {
    ...
    if (operationStatus === caConfig.BUTTON_ENTITY.OPERATION_STATUS.PENDING) {
      ...
      var err = new caErrors.BadOperationStatus(
  ...
  if (request && request.payload && request.payload.contextResponses &&
    ...
    request.payload.contextResponses.forEach(function(contextResponse) {
      if (contextResponse.contextElement) {
        var p1Attr = caHelper.getAttribute(contextResponse.contextElement.attributes, caConfig.DEVICE_ENTITY.P1),
            p1Value = p1Attr.value;
        ...
        if (p1Value) {
          ...
          cellIdDescriptor.cellId = caHelper.getAttributeValue(p1Value, caConfig.DEVICE_ENTITY.P1S.CELL_ID);
          cellIdDescriptor.lac = caHelper.getAttributeValue(p1Value, caConfig.DEVICE_ENTITY.P1S.LAC);
          cellIdDescriptor.mcc = caHelper.getAttributeValue(p1Value, caConfig.DEVICE_ENTITY.P1S.MCC);
          cellIdDescriptor.mnc = caHelper.getAttributeValue(p1Value, caConfig.DEVICE_ENTITY.P1S.MNC);
      ...
      cellTowers: [
        {
          ...
          mobileCountryCode: parseInt(cellIdDescriptor.mcc, 10),
          mobileNetworkCode: parseInt(cellIdDescriptor.mnc, 10)
        }
      ]
    }
  };
      ...
      attributes: [
        {
          ...
          metadatas: [
            {
              name: caConfig.DEVICE_ENTITY.POSITION_METADATA_NAME,
              type: caConfig.DEVICE_ENTITY.POSITION_METADATA_TYPE,
              value: caConfig.DEVICE_ENTITY.POSITION_METADATA_VALUE
            },
            {

  ...
  if (data) {
    ...
    for (var key in detailsObj) {
      if (detailsObj.hasOwnProperty(key)) {
        result += encodeResponse(key + '-' + detailsObj[key] + ';');
      }
    }
    return {
    ...
    contextElements[0].attributes.push(
      {
        ...
        value: JSON.stringify(getResult(null, response))
      }
    );
  }
  ...
  var contextElements = [
    {
      ...
      attributes: [
        ...
        },
        {
          name: caConfig.BUTTON_ENTITY.OPERATION_RESULT_ATTR_NAME,
          type: caConfig.ATTRIBUTE_TYPE.STRING,
          value: JSON.stringify(getResult(null, thirdPartyUpdateRequest))
        }
      ]
    }
  ];
    ...
    contextElements: [
      {
        ...
        attributes: [
          ...
          },
          {
            name: caConfig.BUTTON_ENTITY.OPERATION_RESULT_ATTR_NAME,
            type: caConfig.ATTRIBUTE_TYPE.STRING,
            value: JSON.stringify(getResult(err))
          }
        ]
      }
    ],

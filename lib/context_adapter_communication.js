 ...
 * @param {object} data The request received from the Third Party as an update to an asynchronous request
 * @param {function} callback Callback to be called informing of any errors or returning
 *  the required button interaction type
 */
function getButtonInteractionType(data, callback) {
  var interactionType,
      payload = {
        entities: [
          {
            id: data.payload.buttonId,
  ...
  caLogger.debug(
    data.contextAdapter.context,
    'Sending queryContext to the Context Broker to get the interaction_type attribute of the button with id \'' +
    data.payload.buttonId + '\' using payload: ' +
    JSON.stringify(payload)
  );

  sendQueryContext(
    data.headers['fiware-service'],
    data.headers['fiware-servicepath'],
    payload,
...
function notifyOperationCompleted(request, data, callback) {
  ...
  /**
   * Adds the op_status and aux_op_result attributes to the array of attributes to send to the Context Broker in the
   *  updateContext request for the synchronous button case
   */
  function addSynchronousButtonAttributes() {
  ...
  if (request && request.contextAdapter.operationDescriptor.interactionType ===
    caConfig.BUTTON_ENTITY.INTERACTION_TYPES.SYNCHRONOUS) {
    addSynchronousButtonAttributes();
    doSendUpdateContext();
  } else {
    getButtonInteractionType(data, function(err, interactionType) {
      if (err && callback) {
        return process.nextTick(callback.bind(null, err));
      }
      if (interactionType === caConfig.BUTTON_ENTITY.INTERACTION_TYPES.SYNCHRONOUS) {
        ...
        var badInteractionType = new caErrors.BadInteractionType(data.payload.buttonId, interactionType);

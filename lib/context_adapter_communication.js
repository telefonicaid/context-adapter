...
function getButtonInteractionType(thirdPartyUpdateRequest, callback) {
  ...
  caLogger.debug(
    ...
    'Fiware-Service: ' + thirdPartyUpdateRequest.headers[caConfig.HEADER.FIWARE_SERVICE] + ' and ' +
    'Fiware-ServicePath: ' + thirdPartyUpdateRequest.headers[caConfig.HEADER.FIWARE_SERVICE_PATH] +
    ' and using payload: ' +
    JSON.stringify(payload)
  );

  sendQueryContext(
    thirdPartyUpdateRequest.headers[caConfig.HEADER.FIWARE_SERVICE],
    thirdPartyUpdateRequest.headers[caConfig.HEADER.FIWARE_SERVICE_PATH],
    payload,
...
function getServiceDescriptor(request, callback) {
  ...
  sendQueryContext(
    request.headers[caConfig.HEADER.FIWARE_SERVICE],
    request.headers[caConfig.HEADER.FIWARE_SERVICE_PATH],
    payload,
    function(err, response, body) {
      caLogger.debug(
        request.contextAdapter.context,
